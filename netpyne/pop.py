...
class Pop (object):
    ...
    def createCells(self):
        ...
        # not enough tags to create cells
            ...
            print 'Warning: number or density of cells not specified for population %s; defaulting to numCells = 1' % (self.tags['pop'])
            cells = self.createCellsFixedNum()

        # initialize membrane potentials (if provided)
        if 'vInit' in self.tags:

            vrange = self.tags['vInit']
            ...
            for cell in cells:
                ...
                for key in cell.secs:
                    cell.secs[key].update({'vinit' : v})
