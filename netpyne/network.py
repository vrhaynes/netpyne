...
class Network (object):
    ...
    def traubCellConn(self, preCellsTags, postCellsTags, connParam):
        ...
        # Create array of the total connections (connParam['numPreToPost']) for each post cell (postGid) with elements randomly sampled from preCellsTags
        numPreToPost = connParam['numPreToPost']
        preGids = np.array(preCellsTags.keys())

        connMap = {(connId,postGid): np.random.choice(preGids) for connId in np.arange(numPreToPost) for postGid in postCellsTags} # Could do this better
        ...
        for postCellGid,postCellTags in postCellsTags.iteritems(): # for each post synaptic cell
            if postCellGid in self.lid2gid:  # check if postsyn is in this node
                for connId in np.arange(numPreToPost):
                    # get presynaptic cell info
                    preCellGid = connMap[connId,postCellGid]
