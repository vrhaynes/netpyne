...
def plotRaster (include = ['allCells'], timeRange = None, maxSpikes = 1e8, orderBy = 'gid', orderInverse = False, labels = 'legend', popRates = False,
    ...
    Raster plot of network cells
        ...
        - syncLines (True|False): calculate synchrony measure and plot vertical lines for each spike to evidence synchrony (default: False)
...
def plotSpikeHist (include = ['allCells', 'eachPop'], timeRange = None, binSize = 5, overlay=True, graphType='line', yaxis = 'rate',
    ...
    # colors
    popColorsTmp = {popLabel: colorList[ipop%len(colorList)] for ipop,popLabel in enumerate(include)} # dict with color for each pop
    if popColors: popColorsTmp.update(popColors)
    popColors = popColorsTmp


    # Y-axis label
    if yaxis == 'rate': yaxisLabel = 'Avg firing rate (Hz)'
...
def plotRatePSD (include = ['allCells', 'eachPop'], timeRange = None, binSize = 5, Fs = 200, smooth = 0, overlay=True, ylim = None,
    ...
    for iplot,subset in enumerate(include):
        ...
        if iplot == 0:
            ...
            plt.ylabel('PSD Magnitude (dB)', fontsize=fontsiz) # add yaxis in opposite side
        plt.xlim([0, (Fs/2)-1])
...
def plotTraces (include = None, timeRange = None, overlay = False, oneFigPer = 'cell', rerun = False, colors = None, ylim = None, axis='on',
    ...
    # Plot one trace for each population
    elif oneFigPer == 'popExTrace':
        allPopGids = invertDictMapping(gidPops)
        exGids = []
        for _, popGids in allPopGids.iteritems():
            exGids.append(popGids[0]) # grab first cell

        exGids.reverse()
        plotFigPerTrace(exGids)
